-- Ceramic (and other) funnels store model year as car_year in snapshot payload.
-- Mirror it onto leads.vehicle_year when the snapshot column is empty.
...
declare
  ...
  snapshot_vehicle_year text := nullif(
    trim(
      coalesce(
        new.vehicle_year,
        new.payload ->> 'vehicle_year',
        new.payload ->> 'car_year',
        ''
      )
    ),
    ''
  ...
  if matched_lead_id is null then
    ...
    set primary_session_id = coalesce(primary_session_id, new.session_id),
        ...
        vehicle_model = coalesce(
          ...
          nullif(trim(coalesce(new.payload ->> 'vehicle_model', '')), ''),
          vehicle_model
        ),
        vehicle_year = coalesce(snapshot_vehicle_year, vehicle_year),
