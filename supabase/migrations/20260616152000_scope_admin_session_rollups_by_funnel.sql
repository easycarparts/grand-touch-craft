...
with event_rows as (
  select
    ...
    bool_or(e.event_name in ('price_viewed', 'guided_price_revealed')) as price_viewed,
    bool_or(
      e.event_name in (
        'calculator_started',
    ...
    max(
      coalesce(
        (e.payload ->> 'final_price')::numeric,
        (e.payload ->> 'service_price')::numeric,
        (e.payload ->> 'estimate_value')::numeric
      )
    ) filter (
      where e.payload ? 'final_price'
         or e.payload ? 'service_price'
         or e.payload ? 'estimate_value'
    ) as quote_estimate,
  ...
  group by e.session_id, e.funnel_name, e.landing_page_variant, e.pathname
),
snapshot_rows as (
  select
    s.session_id,
