-- Close-rate stats v2: optional salesperson filter.
-- p_owner: null/'all' = everyone, 'unassigned' = leads with no owner, else admin_users.id::text.
-- The by_owner section always covers all owners (for comparison); everything else respects the filter.
drop function if exists public.admin_close_rate_stats(timestamptz, timestamptz);
