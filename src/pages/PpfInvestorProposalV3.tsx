...
const STORAGE_KEY = "gt-ppf-investor-unlocked-v3";
...
/** Install switch chart: pay installer visas day 1, then save vs 2k/car contractors.
 *  Tracks both install-only cash and the all-in cost (incl. the support team, which is
 *  identical on both paths) so the chart can show full operating cost per path. */
function buildInstallSwitchChart(cpw: number) {
  const e = installEconomics(cpw, "upfront");
  // Support is the same on both paths; included in the "full" lines for a complete picture.
  const supportMonthly = e.supportSalary;
  const supportDay1 = e.supportVisaUpfront;
  let cumContractor = 0;
  let cumInhouse = e.installerVisaUpfront;
  let cumContractorFull = supportDay1;
  let cumInhouseFull = supportDay1 + e.installerVisaUpfront;
  const rows: Array<{
    month: string;
    monthNum: number;
    contractorInstallCum: number;
    inhouseInstallCum: number;
    contractorFullCum: number;
    inhouseFullCum: number;
  ...
  rows.push({
    ...
    contractorFullCum: Math.round(cumContractorFull),
    inhouseFullCum: Math.round(cumInhouseFull),
  ...
  for (let m = 1; m <= 12; m += 1) {
    ...
    cumContractorFull += supportMonthly + e.contractorInstall;
    cumInhouseFull += supportMonthly + e.inhouseInstallPayroll;
    rows.push({
      month: `M${m}`,
      monthNum: m,
 ...
 * call this same function so no two numbers on the page can contradict.
 * ------------------------------------------------------------------------- */
...
export default function PpfInvestorProposalV2() {
  ...
  const milestones = [
    {
      icon: Coins,
      label: "First AED 100k",
      month: timeline.rows.find((r) => r.cumulative >= 100000)?.monthNum ?? null,
      hit: currentRow.cumulative >= 100000,
      threshold: 100000,
    },
    {
      icon: ShieldCheck,
    ...
    {
      icon: Trophy,
      label: "2x your capital",
      month: timeline.doubledMonth,
      hit: currentRow.cumulative >= SETUP_TOTAL * 2,
      threshold: SETUP_TOTAL * 2,
    },
  ];

  useEffect(() => {
    document.title = "Private PPF Investor Deck v3 | Grand Touch";
  ...
  return (
    <main className="min-h-screen bg-[#070707] text-white">
      <style>{`.hide-scrollbar::-webkit-scrollbar{display:none}.hide-scrollbar{scrollbar-width:none}
        ...
        input[type=range].gt-range-lime::-webkit-slider-thumb{border-color:#a3e635}
        input[type=range].gt-range-lime::-moz-range-thumb{border-color:#a3e635}
        @keyframes gtStamp{0%{opacity:0;transform:rotate(-8deg) scale(1.8)}60%{opacity:1;transform:rotate(-8deg) scale(0.92)}100%{opacity:1;transform:rotate(-8deg) scale(1)}}
        .gt-stamp{animation:gtStamp .55s cubic-bezier(.2,.9,.3,1.2) both}
      ...
      <section id="why" className="scroll-mt-20 bg-[#0c0c10] px-5 py-14 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <Reveal className="mb-8 max-w-3xl">
      ...
      {/* ===================== REVENUE PATH CHART ===================== */}
      <section className="bg-[#0c0c10] px-5 py-14 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl">
      ...
      <section id="staffing" className="scroll-mt-20 bg-[#0c0c10] px-5 py-14 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <Reveal className="mb-8 max-w-3xl">
          ...
          {/* ---- Two-phase install model: the visual story ---- */}
          <Reveal className="mb-8">
            <div className="grid items-stretch gap-3 md:grid-cols-[1fr_auto_1fr]">
              {/* Phase 1 — Contractors (now) */}
              <div className="relative overflow-hidden rounded-2xl border border-[#ef6345]/40 bg-[#ef6345]/[0.06] p-5 sm:p-6">
                <span className="absolute right-4 top-4 rounded-full bg-[#ef6345] px-2.5 py-0.5 text-[10px] font-black uppercase tracking-[0.14em] text-black">
                  Now
                </span>
                <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-[#ef6345]/15 text-[#ef6345]">
                  <Wrench className="h-5 w-5" />
                </div>
                <p className="mt-4 text-[11px] font-bold uppercase tracking-[0.18em] text-[#ef6345]">Phase 1 · Launch</p>
                <h3 className="mt-1 text-xl font-black">Install on contractors</h3>
                <p className="mt-2 text-sm leading-6 text-white/65">
                  A flat <strong className="text-white">AED {CONTRACTOR_PER_CAR.toLocaleString()}/car</strong>, billed
                  inside COGS. Cost flexes with volume - no fixed payroll risk while demand is still being proven.
                </p>
                <ul className="mt-4 space-y-2 text-xs text-white/60">
                  <li className="flex items-center gap-2"><Check className="h-4 w-4 shrink-0 text-[#ef6345]" /> Variable - pay only per car completed</li>
                  <li className="flex items-center gap-2"><Check className="h-4 w-4 shrink-0 text-[#ef6345]" /> Zero salaries or visas to carry</li>
                  <li className="flex items-center gap-2"><Check className="h-4 w-4 shrink-0 text-[#ef6345]" /> Scales instantly with bookings</li>
                </ul>
              </div>

              {/* Crossover */}
              <div className="flex items-center justify-center gap-3 md:flex-col">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full border border-white/15 bg-black/50 text-white/80">
                  <ChevronRight className="h-6 w-6 rotate-90 md:rotate-0" />
                </div>
                <p className="max-w-[150px] text-xs font-semibold leading-5 text-white/55 md:text-center">
                  Switch once volume is steady &amp; predictable
                </p>
              </div>

              {/* Phase 2 — In-house (at scale) */}
              <div className="relative overflow-hidden rounded-2xl border border-[#42d6c9]/40 bg-[#42d6c9]/[0.06] p-5 sm:p-6">
                <span className="absolute right-4 top-4 rounded-full border border-[#42d6c9]/50 bg-[#42d6c9]/10 px-2.5 py-0.5 text-[10px] font-black uppercase tracking-[0.14em] text-[#42d6c9]">
                  At scale
                </span>
                <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-[#42d6c9]/15 text-[#42d6c9]">
                  <Users className="h-5 w-5" />
                </div>
                <p className="mt-4 text-[11px] font-bold uppercase tracking-[0.18em] text-[#42d6c9]">Phase 2 · At scale</p>
                <h3 className="mt-1 text-xl font-black">Bring install in-house</h3>
                <p className="mt-2 text-sm leading-6 text-white/65">
                  Salaried installers replace the per-car fee. Once bays run full, fixed payroll beats contractor cost -
                  with tighter quality control and margin kept in the business.
                </p>
                <ul className="mt-4 space-y-2 text-xs text-white/60">
                  <li className="flex items-center gap-2"><Check className="h-4 w-4 shrink-0 text-[#42d6c9]" /> Lower cost per car at steady volume</li>
                  <li className="flex items-center gap-2"><Check className="h-4 w-4 shrink-0 text-[#42d6c9]" /> Full control of finish &amp; QC</li>
                  <li className="flex items-center gap-2"><Check className="h-4 w-4 shrink-0 text-[#42d6c9]" /> Installer visas pay back in weeks</li>
                </ul>
              </div>
            </div>

            {/* Live tie-in to the slider below */}
            <div className="mt-3 flex flex-wrap items-center justify-center gap-x-2 gap-y-1 rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-center text-sm text-white/70">
              <span>At <strong className="text-white">{staffCpw} cars/week</strong>, in-house install runs</span>
              <strong className="text-[#42d6c9]">{formatShort(staff.inhouseInstallPayroll)}/mo</strong>
              <span>vs</span>
              <strong className="text-[#ef6345]">{formatShort(staff.contractorInstall)}/mo</strong>
              <span>on contractors -</span>
              <strong style={{ color: staff.installSavingsUpfront > 0 ? "#a3e635" : "#f8b84e" }}>
                {staff.installSavingsUpfront > 0
                  ? `in-house saves ${formatShort(staff.installSavingsUpfront)}/mo`
                  : "contractors still cheaper"}
              </strong>
            </div>
          </Reveal>
          ...
          <div className="grid gap-6 lg:grid-cols-[0.92fr_1.08fr]">
            ...
            <Reveal className="rounded-2xl border border-white/10 bg-[#101010] p-5 sm:p-6">
                ...
                <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-end">
                  <div>
                    ...
                    <input
                      ...
                      style={{
                        background: `linear-gradient(90deg,#ef6345 ${((staffCpw - 1) / 14) * 100}%, rgba(255,255,255,0.12) ${((staffCpw - 1) / 14) * 100}%)`,
                        accentColor: "#ef6345",
                      }}
                    />
                ...
                <div className="flex gap-2">
                  ...
                  <h3 className="text-lg font-black uppercase tracking-wide">
                    {visaAccounting === "upfront"
                      ? "Upfront visa payback"
                      : "Monthly cost by volume"}
                  </h3>
                  <p className="mt-1 text-xs leading-5 text-white/50">
                    {visaAccounting === "upfront"
                      ? `All-in cost on each path at ${staffCpw} cars/week - both carry the same support team. In-house starts higher (installer visas on Day 1), then pulls below as the gap to contractors compounds.`
              ...
              <div className="mb-3 flex flex-wrap gap-3 text-xs text-white/58">
                <span className="inline-flex items-center gap-2">
                  <span className="h-2 w-4 rounded-full bg-[#ef6345]" />{" "}
                  {visaAccounting === "upfront" ? "Contractor (incl. support)" : "Contractor install"}
                </span>
                <span className="inline-flex items-center gap-2">
                  <span className="h-2 w-4 rounded-full bg-[#42d6c9]" />{" "}
                  {visaAccounting === "upfront" ? "In-house (incl. support)" : "In-house install"}
                </span>
                  ...
                  {visaAccounting === "upfront" ? (
                    <ComposedChart data={staffCumulative.rows} margin={{ top: 20, right: 12, left: 0, bottom: 0 }}>
                      ...
                      <Tooltip
                        ...
                        labelFormatter={(label) => (label === "Day 1" ? "Day 1 · visas paid upfront" : `${label} · all-in cost so far`)}
                        formatter={(value: number, name: string) => {
                          const labels: Record<string, string> = {
                            contractorFullCum: "Contractor (incl. support)",
                            inhouseFullCum: "In-house (incl. support)",
                          };
                      ...
                      {staffCumulative.paybackMonth ? (
                        <ReferenceLine
                          ...
                          strokeWidth={2}
                          label={{
                            value: `In-house wins · M${staffCumulative.paybackMonth}`,
                            position: "top",
                            fill: "#a3e635",
                            fontSize: 11,
                            fontWeight: 700,
                      ...
                      <Line
                        type="monotone"
                        dataKey="contractorFullCum"
                        stroke="#ef6345"
                        strokeWidth={3}
                        dot={false}
                        name="contractorFullCum"
                      />
                      <Line
                        type="monotone"
                        dataKey="inhouseFullCum"
                        stroke="#42d6c9"
                        strokeWidth={3}
                        dot={false}
                        name="inhouseFullCum"
                ...
                {visaAccounting === "upfront" ? (
                  <>
                    <div className="rounded-lg border border-[#ef6345]/25 bg-[#ef6345]/[0.07] p-2.5">
                      <p className="text-[10px] uppercase tracking-[0.14em] text-white/45">12-mo contractor cost</p>
                      <p className="mt-1 text-lg font-black text-[#ef6345]">
                        {formatAed(staffCumulative.month12?.contractorFullCum ?? 0)}
                      </p>
                      <p className="text-[10px] text-white/40">all-in, incl. support</p>
                    </div>
                    <div className="rounded-lg border border-[#42d6c9]/25 bg-[#42d6c9]/[0.07] p-2.5">
                      <p className="text-[10px] uppercase tracking-[0.14em] text-white/45">12-mo in-house cost</p>
                      <p className="mt-1 text-lg font-black text-[#42d6c9]">
                        {formatAed(staffCumulative.month12?.inhouseFullCum ?? 0)}
                      </p>
                      <p className="text-[10px] text-white/40">all-in, incl. support + visas</p>
                    </div>
                    <div
                      className={cx(
                        "rounded-lg p-2.5",
              ...
              <div className="mt-3 flex items-center gap-3 rounded-lg border border-[#f8b84e]/25 bg-[#f8b84e]/[0.07] p-3.5">
                <KeyRound className="h-5 w-5 shrink-0 text-[#f8b84e]" />
                <p className="text-sm leading-6 text-white/80">
                  {staff.installSavingsUpfront > 0 ? (
                    <>
                      <strong className="text-white">In-house wins at {staffCpw} cars/week</strong> - it saves{" "}
                      <strong className="text-[#a3e635]">{formatShort(staff.installSavingsUpfront)}/mo</strong> on install, and the{" "}
                      {formatShort(staff.installerVisaUpfront)} in visas
                      {staffCumulative.paybackMonth ? (
                        <> pays back by <strong className="text-white">M{staffCumulative.paybackMonth}</strong>.</>
                      ) : (
                        <> pays back fast.</>
                      )}{" "}
                      Until volume is steady, contractors stay the safer call.
                    </>
                  ) : (
                    <>
                      <strong className="text-white">Stay on contractors at {staffCpw} cars/week.</strong> Volume is too
                      low for salaried installers to beat AED {CONTRACTOR_PER_CAR.toLocaleString()}/car - revisit as it grows.
                    </>
                  )}
                </p>
              </div>
      ...
      {/* ===================== YOUR RETURN — gamified payoff ===================== */}
      <section id="returns" className="relative scroll-mt-20 overflow-hidden px-5 py-16 sm:px-8 lg:px-10">
        {/* Accent band: this is the climax, so it breaks the page rhythm */}
        <div className="absolute inset-0 bg-[radial-gradient(120%_120%_at_50%_-10%,rgba(163,230,53,0.12),transparent_55%),radial-gradient(120%_120%_at_100%_120%,rgba(66,214,201,0.10),transparent_55%)]" />
        <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#a3e635]/40 to-transparent" />
        <div className="relative mx-auto max-w-7xl">
          <Reveal className="mb-8 max-w-3xl">
          ...
          <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
            {/* Money counter + recovery meter + scrubber */}
            <Reveal className="relative flex flex-col gap-6 rounded-2xl border border-[#a3e635]/25 bg-[#0c100a] p-6 sm:p-7">
              {/* Big animated cash counter */}
              <div className="relative">
                <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-white/45">
                  <Wallet className="h-4 w-4 text-[#a3e635]" />
                  Cash in your pocket by {currentRow.month === "Start" ? "day one" : currentRow.month}
                </p>
                <p
              ...
              <div>
                ...
                <div className="relative h-4 overflow-hidden rounded-full border border-white/10 bg-white/[0.04]">
                  ...
                  <span
                    ...
                    style={{ left: `${(100000 / SETUP_TOTAL) * 100}%` }}
                  />
                </div>
              ...
              <div>
                <div className="flex items-end justify-between">
                  <label className="text-sm font-bold uppercase tracking-[0.18em] text-white/55">Drag the timeline</label>
                  <span className="text-2xl font-black text-[#a3e635]">
                    {currentRow.month === "Start" ? "Day 1" : `Month ${returnMonth}`}
                  </span>
                </div>
                <input
                  type="range"
                  ...
                  style={{
                    background: `linear-gradient(90deg,#a3e635 ${(returnMonth / 24) * 100}%, rgba(255,255,255,0.12) ${(returnMonth / 24) * 100}%)`,
                  }}
                />
                <div className="mt-2 flex justify-between text-xs text-white/45">
                  <span>Day 1</span>
                ...
                {milestones.map((m) => (
                  <div
                    ...
                    className={cx(
                      ...
                      m.hit
                        ? "gt-pop border-[#a3e635]/40 bg-[#a3e635]/10"
                        : "border-white/10 bg-black/30 opacity-55"
                    )}
                  >
                    <m.icon
                      className={cx("mx-auto h-5 w-5", m.hit ? "text-[#a3e635]" : "text-white/40")}
                    />
                    <p className="mt-1.5 text-[10px] font-bold uppercase tracking-[0.1em] text-white/70">{m.label}</p>
                    <p className={cx("text-[10px] font-black", m.hit ? "text-[#a3e635]" : "text-white/35")}>
                      {m.month ? `M${m.month}` : "—"}
                    </p>
                  </div>
                ))}
            ...
            {/* Scenario chart */}
            <Reveal className="rounded-2xl border border-white/10 bg-[#0c0c0e] p-5 sm:p-6">
              <div className="mb-3 flex items-center gap-2">
                <LineChartIcon className="h-5 w-5 text-[#a3e635]" />
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                ...
                <div className="rounded-lg border border-white/10 bg-black/25 p-2.5">
                  <p className="text-[10px] uppercase tracking-[0.12em] text-white/45">By month 24</p>
                  <p className="mt-0.5 text-base font-black text-white">{formatShort(timeline.total)}</p>
                </div>
              </div>
            </Reveal>
          </div>
          ...
          {/* Climax line */}
          <Reveal className="mt-6 overflow-hidden rounded-2xl border border-[#a3e635]/30 bg-[linear-gradient(135deg,rgba(163,230,53,0.10),transparent_60%)] p-6 sm:p-8">
          ...
          {/* Risk frame (replaces the buyout claim) + soft exit note */}
          <div className="mt-6 grid gap-4 lg:grid-cols-3">
            {[
              {
                icon: Wrench,
                title: "Owned, re-sellable assets",
                body: "Tools, plotter and stock are already paid for and hold resale value - real downside protection if plans ever change.",
              },
              {
                icon: Scale,
                title: "A variable cost base",
                body: "Install runs on contractors at AED 2k/car, so cost flexes with volume. Low fixed risk while the studio ramps.",
              },
              {
                icon: TrendingUp,
                title: "Pricing headroom",
                body: "Today's AED 10-12k sits below premium-market rates. The brand can raise price over time - upside the floor doesn't even count.",
              },
            ].map((card, i) => (
              <Reveal key={card.title} delay={i * 70} className="rounded-2xl border border-white/10 bg-[#0c0c0e] p-6">
                <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-md bg-[#a3e635]/12 text-[#a3e635]">
                  <card.icon className="h-5 w-5" />
                </div>
          ...
          <Reveal className="mt-4 flex items-start gap-3 rounded-2xl border border-white/10 bg-[#0c0c0e] p-5">
            <KeyRound className="mt-0.5 h-5 w-5 shrink-0 text-[#f8b84e]" />
            <p className="text-sm leading-7 text-white/65">
              <strong className="text-white">Optional future buyout.</strong> If both sides want it, Sean can buy back
              the investor's share at a later date - on terms agreed together at the time. No fixed multiple and no
              obligation; the partnership can simply continue as a passive income share for as long as it suits you.
            </p>
          </Reveal>
        </div>
      </section>
      ...
      <section id="ask" className="scroll-mt-20 px-5 py-14 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl">
                      ...
                      <PieChart>
                        ...
                        >
                          {costBuckets.map((line) => (
                            <Cell key={line.name} fill={line.color} />
                          ))}
                        </Pie>
                        <Tooltip
                          contentStyle={{ background: "#0b0b0b", border: "1px solid rgba(255,255,255,0.14)", borderRadius: 8 }}
                          formatter={(value: number, name: string) => [
                            `${formatAed(value)} · ${Math.round((value / SETUP_TOTAL) * 100)}%`,
                            name,
                          ]}
                        />
                      </PieChart>
                    ...
                    {costBuckets.map((line) => (
                      <div key={line.name} className="flex items-center justify-between gap-3 text-xs">
                        ...
                        <span className="shrink-0 font-semibold text-white/55">
                          {formatShort(line.value)} · {Math.round((line.value / SETUP_TOTAL) * 100)}%
                        </span>
                      </div>
                    ))}
                  </div>
          ...
          {/* Already covered */}
          <Reveal className="mt-8 rounded-2xl border border-[#a3e635]/25 bg-[#a3e635]/[0.05] p-6 sm:p-8">
            <div className="mb-5 flex flex-wrap items-center gap-3">
              <span className="inline-flex h-9 items-center gap-2 rounded-full bg-[#a3e635] px-4 text-xs font-black uppercase tracking-[0.14em] text-black">
                <Check className="h-4 w-4" /> Already paid for
              </span>
              <p className="text-lg font-black sm:text-xl">
                The expensive parts most workshops fund - <span className="text-[#a3e635]">AED 0 from you</span>.
              </p>
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              {ownedAssets.map((asset) => (
                <div key={asset.item} className="flex gap-4 rounded-lg border border-white/10 bg-black/30 p-5">
      ...
      {/* ===================== 12-MONTH ROADMAP ===================== */}
      <section id="roadmap" className="scroll-mt-20 bg-[#0c0c10] px-5 py-14 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <Reveal className="mb-8 max-w-3xl">
      ...
      {/* ===================== THE DEAL STRUCTURE ===================== */}
      <section id="deal" className="scroll-mt-20 bg-[#0c0c10] px-5 py-14 sm:px-8 lg:px-10">
        <div className="mx-auto max-w-7xl">
          <Reveal className="mb-8 max-w-3xl">
          ...
          <Reveal className="mb-6 grid gap-4 md:grid-cols-2">
            ...
            <div className="rounded-2xl border border-[#42d6c9]/30 bg-[#42d6c9]/[0.06] p-6">
              <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-[#42d6c9]/15 text-[#42d6c9]">
