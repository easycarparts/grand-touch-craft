...
import { CalendarDays, CheckCircle2, MapPin, MessageCircle, ShieldCheck, Sparkles } from "lucide-react";
...
import {
  ...
  confirmCeramicBooking,
  formatAed,
  formatDubaiDate,
  formatDubaiDateTime,
  loadCeramicDateAvailability,
  loadPublicCeramicBooking,
  todayDubaiDate,
} from "@/lib/ceramic-bookings";

const shopWhatsApp = "https://wa.me/971567191045";

const getVehicle = (booking: CeramicBooking) =>
  booking.vehicle_label || [booking.vehicle_year, booking.vehicle_make, booking.vehicle_model].filter(Boolean).join(" ");

const getAddOnLabels = (booking: CeramicBooking) =>
  (booking.add_ons || [])
    .map((item) => (typeof item === "string" ? item : (item as { title?: string }).title || ""))
    .filter(Boolean);

const statusCopy = (status: CeramicBooking["status"]) => {
  if (status === "confirmed") return "Your ceramic booking is confirmed.";
  if (status === "completed") return "This ceramic booking has been completed.";
  if (status === "opened") return "Your ceramic bay day is held. Confirm below.";
  return "Your ceramic bay day is ready to confirm.";
};
...
function ConfirmedView({ booking }: { booking: CeramicBooking }) {
      ...
      <main className="mx-auto max-w-2xl">
        <div className="rounded-[28px] border border-emerald-400/20 bg-[radial-gradient(circle_at_top,rgba(16,185,129,0.22),transparent_36%),linear-gradient(180deg,rgba(255,255,255,0.05),rgba(10,10,10,0.98))] p-6 text-center shadow-[0_24px_80px_rgba(0,0,0,0.35)] sm:p-8">
          ...
          <div className="mt-6 rounded-2xl border border-white/10 bg-black/30 p-4 text-left">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Ceramic bay</p>
                <p className="mt-1 font-semibold">9 AM - 6 PM</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="mt-1 font-semibold">{booking.package_name}</p>
              </div>
            </div>
          </div>
        ...
        <section className="mt-5 rounded-2xl border border-white/10 bg-white/[0.04] p-5">
          ...
          <ol className="mt-3 space-y-2 text-sm leading-6 text-slate-300">
            <li>1. Sean keeps the ceramic bay assigned to your car.</li>
            <li>2. Paint condition is inspected before final correction level is confirmed.</li>
            <li>3. Drop-off is usually between 9 AM and 11 AM unless agreed otherwise.</li>
          </ol>
        </section>
      </main>
    </div>
...
const CeramicBookingConfirmation = () => {
  const { token } = useParams<{ token: string }>();
  const [booking, setBooking] = useState<CeramicBooking | null>(null);
  ...
  const [confirmedBooking, setConfirmedBooking] = useState<CeramicBooking | null>(null);

  useEffect(() => {
    if (!token) return;
    (async () => {
      try {
        const data = await loadPublicCeramicBooking(token);
        setBooking(data);
  ...
  useEffect(() => {
    ...
    loadCeramicDateAvailability(token, selectedDate)
      .then((slots) => {
        setDateAvailable(slots[0]?.available ?? true);
      })
      .catch((availabilityError) => {
        setSubmitError(availabilityError instanceof Error ? availabilityError.message : "Could not check day availability.");
      })
      .finally(() => setAvailabilityLoading(false));
  }, [booking, confirmedBooking, selectedDate, token]);

  const vehicle = booking ? getVehicle(booking) : "";
  const addOns = booking ? getAddOnLabels(booking) : [];
  const canConfirm = customerName.trim().length >= 2 && agreed && Boolean(selectedDate);
  const minDate = todayDubaiDate();
  const maxDate = useMemo(() => addDaysToDateString(minDate, 45), [minDate]);

  const handleConfirm = async () => {
    if (!token || !canConfirm || !dateAvailable || submitting) return;
    setSubmitting(true);
    setSubmitError(null);
    try {
      const nextBooking = await confirmCeramicBooking(token, customerName.trim(), selectedDate);
      setConfirmedBooking(nextBooking);
      window.scrollTo({ top: 0, behavior: "smooth" });
    } catch (confirmError) {
      setSubmitError(confirmError instanceof Error ? confirmError.message : "Could not confirm this day. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#090909] pb-24 text-white">
      <header className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-3xl px-4 pb-8 pt-12 sm:px-6 sm:pt-16">
          <Badge variant="outline" className="border-primary/30 bg-primary/10 text-primary">
            Ceramic bay reserved for {booking.customer_name}
          </Badge>
          <h1 className="mt-5 text-4xl font-black leading-tight tracking-tight sm:text-5xl">
            Confirm your GYEON ceramic booking
          </h1>
          <p className="mt-4 max-w-2xl text-base leading-7 text-slate-300">
            Review the details, keep this date, or choose another available ceramic bay day before confirming.
          </p>
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
              <Sparkles className="h-5 w-5 text-primary" />
              <p className="mt-2 text-sm font-semibold">GYEON products</p>
              <p className="text-xs text-slate-400">Package confirmed after inspection.</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
              <CalendarDays className="h-5 w-5 text-primary" />
              <p className="mt-2 text-sm font-semibold">One bay day</p>
              <p className="text-xs text-slate-400">9 AM to 6 PM held for your car.</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
              <ShieldCheck className="h-5 w-5 text-primary" />
              <p className="mt-2 text-sm font-semibold">Paint checked first</p>
              <p className="text-xs text-slate-400">Correction level depends on condition.</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-3xl space-y-4 px-4 sm:px-6">
        <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
          <h2 className="text-xl font-bold">Your ceramic details</h2>
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.16em] text-slate-500">Current date</p>
              <p className="mt-1 font-semibold">{formatDubaiDateTime(booking.slot_start_at)}</p>
            </div>
          </div>
          {booking.paint_condition ? (
            <div className="mt-4 rounded-xl border border-sky-400/20 bg-sky-500/10 p-3">
              <p className="text-xs uppercase tracking-[0.16em] text-sky-200">Paint condition</p>
              <p className="mt-1 text-sm text-sky-50">{booking.paint_condition}</p>
            </div>
          ) : null}
          {addOns.length ? (
        ...
        <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-400">
            Ceramic is booked as one bay day. If this date no longer works, choose another available day.
          </p>
          <div className="mt-4">
            ...
            <Input
              ...
              className="max-w-sm border-white/10 bg-black/30 text-white"
            />
            <p className="mt-2 text-xs text-slate-500">{formatDubaiDate(`${selectedDate}T08:00:00+04:00`)}</p>
            {availabilityLoading ? <p className="mt-2 text-sm text-slate-400">Checking availability...</p> : null}
            {!dateAvailable ? (
              <p className="mt-2 text-sm text-rose-300">That ceramic bay day has been taken. Please choose another date.</p>
            ) : (
              <p className="mt-2 text-sm text-emerald-300">Available: 9 AM to 6 PM ceramic bay day.</p>
            )}
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-primary/25 bg-[linear-gradient(180deg,rgba(245,181,43,0.12),rgba(255,255,255,0.04))] p-5">
          ...
          <label className="mt-4 flex items-start gap-3 rounded-xl border border-white/10 bg-black/20 p-3 text-sm leading-6 text-slate-300">
            ...
            <span>
              I confirm this ceramic appointment for {formatDubaiDate(`${selectedDate}T08:00:00+04:00`)}. I understand Sean
              will confirm the final correction level after seeing the paint.
            </span>
          </label>
          {submitError ? <p className="mt-3 text-sm text-rose-300">{submitError}</p> : null}
          <Button
            type="button"
            disabled={!canConfirm || !dateAvailable || submitting}
            className="mt-5 w-full bg-primary py-6 text-base font-black text-black hover:bg-primary/90"
            onClick={handleConfirm}
          >
            {submitting ? "Confirming..." : `Confirm ${booking.package_name}`}
          </Button>
        </section>
      ...
      <div className="fixed inset-x-0 bottom-0 border-t border-white/10 bg-black/80 px-4 py-3 backdrop-blur-xl">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm font-semibold">{formatDubaiDate(`${selectedDate}T08:00:00+04:00`)}</p>
          </div>
          <Button
            type="button"
            disabled={!canConfirm || !dateAvailable || submitting}
...
export default CeramicBookingConfirmation;
